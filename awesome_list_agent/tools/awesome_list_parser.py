...
class AwesomeListParserMetadata(BaseModel):
    ...
    description: str = (
        "Parses an Awesome List URL to extract general topic, categories, and contextual information"
    )
...
class AwesomeListParser(BaseTool):
        ...
        try:
            ...
            if hasattr(self.logger, "add_tool_span"):
                ...
                self.logger.add_tool_span(
                    ...
                    success=(
                        "error" not in basic_parsed_data
                        if isinstance(basic_parsed_data, dict)
                        else True
                    ),
                )
            ...
            if hasattr(self.logger, "add_tool_span"):
                ...
                self.logger.add_tool_span(
                    ...
                    success=(
                        "error" not in web_scraping_data
                        if isinstance(web_scraping_data, dict)
                        else True
                    ),
                )
            ...
            youtube_metadata = await self._extract_youtube_metadata(
                basic_parsed_data, web_scraping_data
            )
            ...
            if hasattr(self.logger, "add_tool_span"):
                ...
                self.logger.add_tool_span(
                    ...
                    inputs={
                        "basic_data": basic_parsed_data,
                        "web_data": web_scraping_data,
                    },
            ...
            if hasattr(self.logger, "add_tool_span"):
                ...
                self.logger.add_tool_span(
                    ...
                    success=(
                        "error" not in comprehensive_result
            ...
            self.logger.debug(
                f"Extracted topic: {comprehensive_result.get('topic', 'N/A')}"
            )
            self.logger.debug(
                f"Found {comprehensive_result.get('total_items', 0)} items"
            )
            self.logger.debug(
                f"Detected {len(comprehensive_result.get('categories', []))} categories"
            )
            self.logger.debug(
                f"Found {len(comprehensive_result.get('youtube_metadata', []))} YouTube videos"
            )
    ...
    async def _parse_basic_content(self, url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            async with session.get(url) as response:
                ...
                self.logger.info(
                    f"Successfully fetched content ({content_length} characters)"
                )
    ...
    async def _perform_web_scraping(self, url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(
                f"Web scraping completed - found {len(scraping_result.get('links', []))} links"
            )
    ...
    async def _extract_youtube_metadata(
        self, basic_data: Dict[str, Any], web_data: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for i, youtube_url in enumerate(youtube_urls[:5]):
                try:
                    self.logger.debug(
                        f"Processing YouTube URL {i+1}/{min(len(youtube_urls), 5)}: {youtube_url}"
                    )
                    ...
                    if hasattr(metadata, "error"):
                        self.logger.warning(
                            f"Failed to extract metadata for YouTube URL {youtube_url}: {metadata.error}"
                        )
                        continue
                    elif isinstance(metadata, dict) and "error" not in metadata:
                        youtube_metadata.append(metadata)
                        self.logger.debug(
                            f"Successfully extracted metadata for: {metadata.get('title', 'Unknown')}"
                        )
                    else:
                        self.logger.warning(
                            f"Failed to extract metadata for YouTube URL: {youtube_url}"
                        )
            ...
            self.logger.info(
                f"Successfully extracted metadata for {len(youtube_metadata)} YouTube videos"
            )
            return youtube_metadata
    ...
    async def _create_comprehensive_summary(
        ...
        if (
            "description" in basic_data
            and basic_data["description"] != "No description available"
        ):
        ...
        if "error" not in web_data:
            if "links" in web_data:
                ...
                summary_parts.append(
                    f"The list contains {link_count} curated resources and links."
                )
        ...
        if youtube_data:
            ...
            avg_duration = sum(
                video.get("duration_seconds", 0) for video in youtube_data
            ) / len(youtube_data)

            summary_parts.append(
                f"🎥 Found {video_count} YouTube videos with a total of {total_views:,} views."
            )
            summary_parts.append(
            ...
            if video_count > 0:
                sample_titles = [
                    video.get("title", "Unknown") for video in youtube_data[:3]
                ]
                if sample_titles:
                    summary_parts.append(
                        f"📹 Sample videos: {', '.join(sample_titles)}"
                    )
        else:
        ...
        if "categories" in basic_data and basic_data["categories"]:
            ...
            summary_parts.append(
                f"Content is organized into {cat_count} categories: {categories_text}."
            )

        # Language information
        if "language" in basic_data and basic_data["language"] != "General":
            summary_parts.append(
                f"Primary programming language: {basic_data['language']}."
            )
    ...
    def _extract_topic(self, content: str, url: str) -> str:
        ...
        if h1_match:
            ...
            h1_text = re.sub(
                r"^(awesome-?|Awesome\s+)", "", h1_text, flags=re.IGNORECASE
            )
            if h1_text:
                return h1_text
        ...
        if md_h1_match:
            ...
            md_title = re.sub(
                r"^(awesome-?|Awesome\s+)", "", md_title, flags=re.IGNORECASE
            )
            if md_title:
                return md_title
    ...
    def _extract_categories(self, content: str) -> List[str]:
        ...
        html_headings = re.findall(
            r"<h[23][^>]*>([^<]+)</h[23]>", content, re.IGNORECASE
        )
    ...
    def _count_items(self, content: str) -> int:
        ...
        html_links = len(
            re.findall(r"<a[^>]+href=[^>]*>([^<]+)</a>", content, re.IGNORECASE)
        )
    ...
    def _generate_context_summary(
        ...
        if total_items > 0:
            summary_parts.append(
                f"The list contains approximately {total_items} curated resources."
            )
