...
class YouTubeDownloader:
        ...
        try:
            with YoutubeDL(ydl_opts) as ydl:
                ...
                if language in subtitles and subtitles[language]:
                    transcript_text = self._download_subtitle_content(
    ...
    def _download_subtitle_content(self, subtitle_info: dict[str, Any]) -> str | None:
        ...
        try:
            ...
            if subtitle_info.get("ext") == "vtt":
            ...
            elif subtitle_info.get("ext") == "srv3" or "srv" in subtitle_info.get(
                "ext", ""
            ):
                ...
                clean_content = re.sub(
                    r"^\d+$", "", clean_content, flags=re.MULTILINE
    ...
    def _extract_subtitle_content(self, info: dict[str, Any]) -> dict[str, str]:
        ...
        for lang, sub_list in subtitles.items():
            if sub_list and isinstance(sub_list, list):
                ...
                subtitle_content[f"manual_{lang}"] = (
                    f"Available subtitle formats: {[s.get('ext', 'unknown') for s in sub_list]}"
                )
        ...
        for lang, cap_list in auto_captions.items():
            if cap_list and isinstance(cap_list, list):
                subtitle_content[f"auto_{lang}"] = (
                    f"Available caption formats: {[c.get('ext', 'unknown') for c in cap_list]}"
                )
...
class YouTubeMetadataTool(BaseTool):
    ...
    async def execute(
        self,
        url: str,
        transcript_or_metadata: YoutubeDataType = YoutubeDataType.default,
    ) -> Dict[str, Any]:
        """Extract metadata from a YouTube video URL.

        Args:
            url: The YouTube video URL to extract metadata from
            transcript_or_metadata: Type of data to extract (default, transcript, or metadata)

        Returns:
            Dictionary containing comprehensive video metadata or transcript data
        """
        ...
        try:
            ...
            self.logger.info(
                f"Starting YouTube {transcript_or_metadata.value} extraction for URL: {url}"
            )

            # Use YouTubeDownloader to extract information based on data type
            match transcript_or_metadata:
                case YoutubeDataType.default:
                    self.logger.info(
                        "Fetching all video information using YouTubeDownloader"
                    )
                    youtube_data = self.downloader.get_all_info(url)
                case YoutubeDataType.transcript:
                    self.logger.info(
                        "Fetching video transcript using YouTubeDownloader"
                    )
                    youtube_data = self.downloader.get_transcript(url)
                case YoutubeDataType.metadata:
                    self.logger.info(
                        "Fetching video metadata only using YouTubeDownloader"
                    )
                    youtube_data = self.downloader.get_metadata_only(url)

            # Handle transcript response (which returns a dict instead of YouTubeData)
            if transcript_or_metadata == YoutubeDataType.transcript:
                if isinstance(youtube_data, dict):
                    # Calculate duration
                    duration_ns = time.perf_counter_ns() - start_time

                    # Add timing information
                    youtube_data["extraction_time_ms"] = duration_ns / 1_000_000

                    # Log successful extraction
                    self.logger.info(f"Successfully extracted transcript for video")
                    self.logger.debug(
                    ...
                    error_msg = "Transcript extraction returned unexpected format"
                    self.logger.error(error_msg)
                    return ToolError(error=error_msg)

            # Check if extraction failed for YouTubeData responses
            if hasattr(youtube_data, "error") and youtube_data.error:
                error_msg = f"YouTube extraction failed: {youtube_data.error}"
                self.logger.error(error_msg)
                return ToolError(error=error_msg)

            # Convert YouTubeData to dictionary format expected by the tool
            metadata = self._convert_youtube_data_to_metadata(youtube_data)
            ...
            # Log successful extraction
            self.logger.info(
                f"Successfully extracted {transcript_or_metadata.value} for video: {metadata.get('title', 'Unknown')}"
            )
            self.logger.debug(
                f"Extraction completed in {duration_ns / 1_000_000:.2f}ms"
            )
            ...
            error_msg = f"Unexpected error extracting YouTube {transcript_or_metadata.value}: {str(e)}"
            self.logger.error(error_msg, exc_info=True)
            return ToolError(error=error_msg)

    def _convert_youtube_data_to_metadata(self, youtube_data) -> Dict[str, Any]:
        """Convert YouTubeData model to the expected metadata format.

        Args:
            youtube_data: YouTubeData model from YouTubeDownloader
        ...
        metadata = {
            ...
            "dislike_count": youtube_data.dislike_count,
            "comment_count": youtube_data.comment_count,
            "upload_date": youtube_data.upload_date,
            "channel_name": youtube_data.channel or youtube_data.uploader,
            "channel_id": youtube_data.channel_id or youtube_data.uploader_id,
            "channel_url": youtube_data.channel_url or youtube_data.uploader_url,
            "tags": youtube_data.tags,
            "categories": youtube_data.categories,
            "thumbnail_url": youtube_data.thumbnail,
            "thumbnails": youtube_data.thumbnails,
            "webpage_url": youtube_data.webpage_url,
            "original_url": youtube_data.original_url,
            "width": youtube_data.width,
            "height": youtube_data.height,
            "fps": youtube_data.fps,
            "filesize": youtube_data.filesize,
            "age_limit": youtube_data.age_limit,
            "availability": youtube_data.availability,
            "live_status": youtube_data.live_status,
            "subtitles": youtube_data.subtitles,
            "automatic_captions": youtube_data.automatic_captions,
            "subtitle_content": youtube_data.subtitle_content,
            "formats": youtube_data.formats,
            "playlist": youtube_data.playlist,
            "playlist_id": youtube_data.playlist_id,
            "playlist_title": youtube_data.playlist_title,
            "playlist_index": youtube_data.playlist_index,
            "playlist_count": youtube_data.playlist_count,
        }

        # Create a summary of the metadata
        summary_parts = []
        if youtube_data.view_count:
            summary_parts.append(f"{youtube_data.view_count:,} views")
        if youtube_data.like_count:
            summary_parts.append(f"{youtube_data.like_count:,} likes")
        if youtube_data.comment_count:
            summary_parts.append(f"{youtube_data.comment_count:,} comments")
        if youtube_data.duration:
            minutes, seconds = divmod(youtube_data.duration, 60)
            if minutes >= 60:
                hours, minutes = divmod(minutes, 60)
                summary_parts.append(f"Duration: {hours}h {minutes}m {seconds}s")
            else:
        ...
        # Convert ISO 8601 duration format if needed
        if youtube_data.duration:
            metadata["duration"] = f"PT{youtube_data.duration}S"
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        # No session cleanup needed since we're using YouTubeDownloader
        pass
