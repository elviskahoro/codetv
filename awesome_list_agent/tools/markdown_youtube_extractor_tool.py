...
class MarkdownYouTubeExtractorTool(BaseTool):
    ...
    def _scrape_markdown_content(self, url: str) -> str:
        ...
        try:
            ...
            markdown = scrape_data.get("markdown", "")
            self.logger.info(f"Successfully scraped {len(markdown)} characters of markdown content")
    ...
    async def execute(
        ...
        try:
            ...
            self.logger.info(f"Successfully extracted {len(youtube_urls)} YouTube URLs from {url}")
            return result
