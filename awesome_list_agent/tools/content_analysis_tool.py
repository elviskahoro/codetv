...
class ContentAnalysisToolMetadata(ToolMetadata):
    ...
    description: str = "Analyzes text content to extract insights including sentiment, key topics, entities, readability metrics, and content structure"
    tags: List[str] = ["content-analysis", "text-processing", "nlp", "insights", "sentiment-analysis"]
...
class ContentAnalysisTool(BaseTool):
    ...
    def __init__(self):
        ...
        self.positive_words = {
            ...
            'best', 'perfect', 'outstanding', 'brilliant', 'superb', 'terrific', 'incredible',
            'love', 'like', 'enjoy', 'happy', 'pleased', 'satisfied', 'impressed', 'useful',
            'helpful', 'effective', 'efficient', 'powerful', 'fast', 'easy', 'simple',
            'clear', 'comprehensive', 'detailed', 'thorough', 'complete', 'reliable'
        }
        
        self.negative_words = {
            'bad', 'terrible', 'awful', 'horrible', 'worst', 'poor', 'disappointing',
            'frustrating', 'annoying', 'difficult', 'complicated', 'confusing', 'unclear',
            'slow', 'broken', 'error', 'fail', 'problem', 'issue', 'bug', 'crash',
            'hate', 'dislike', 'unhappy', 'angry', 'upset', 'disappointed', 'useless',
            'ineffective', 'inefficient', 'weak', 'limited', 'incomplete', 'unreliable'
        }
    ...
    async def execute(
        ...
        try:
            ...
            self.logger.debug(f"Analysis options: sentiment={analyze_sentiment}, topics={extract_topics}, entities={extract_entities}, readability={analyze_readability}, keywords={extract_keywords}")
    ...
    def _extract_topics(self, text: str, max_topics: int) -> List[Dict[str, Any]]:
        ...
        stop_words = {
            ...
            'his', 'her', 'its', 'our', 'their', 'mine', 'yours', 'his', 'hers',
            'ours', 'theirs', 'what', 'when', 'where', 'why', 'how', 'who', 'which'
        }
    ...
    def _calculate_readability(self, text: str) -> Dict[str, float]:
        ...
        coleman_liau = (0.0588 * (letters / word_count * 100)) - (0.296 * (sentence_count / word_count * 100)) - 15.8
    ...
    def _analyze_content_structure(self, text: str) -> Dict[str, Any]:
        ...
        # Check for links
        has_links = bool(re.search(r'\[([^\]]+)\]\([^)]+\)', text) or 
                        re.search(r'https?://[^\s]+', text))
        
        # Check for code blocks
        has_code_blocks = bool(re.search(r'```[\s\S]*?```', text) or 
                              re.search(r'`[^`]+`', text))
    ...
    def _generate_analysis_summary(self, result: Dict[str, Any]) -> str:
        ...
        # Basic stats
        summary_parts.append(f"Analyzed {result.get('word_count', 0)} words in {result.get('sentence_count', 0)} sentences")
        
        # Sentiment
        if 'sentiment' in result:
            sentiment = result['sentiment']
            summary_parts.append(f"Overall sentiment is {sentiment['overall']} (score: {sentiment['score']:.3f})")
        ...
        if 'readability' in result:
            ...
            summary_parts.append(f"Readability level: approximately {grade_level:.1f} grade level")
