...
class WebScrapingToolMetadata(ToolMetadata):
    ...
    description: str = "Scrapes web content from URLs and extracts structured information including text, links, images, and metadata"
    tags: List[str] = ["web-scraping", "content-extraction", "html-parsing", "data-collection"]
    ...
    output_schema: Dict[str, Any] = {
        ...
        "properties": {
            ...
            "metadata": {
                "type": "object",
                "description": "Page metadata (meta tags)"
            },
            "content_type": {
                "type": "string",
...
class WebScrapingTool(BaseTool):
    ...
    async def execute(
        ...
        try:
            ...
            self.logger.debug(f"Extraction options: text={extract_text}, links={extract_links}, images={extract_images}, metadata={extract_metadata}")
            ...
            async with session.get(url, timeout=aiohttp.ClientTimeout(total=timeout)) as response:
                ...
                self.logger.info(f"Successfully fetched content ({content_length} characters, type: {content_type})")
            ...
            result["title"] = title_tag.get_text().strip() if title_tag else "No title found"
    ...
    def _extract_links(self, soup: BeautifulSoup, max_links: int) -> List[Dict[str, str]]:
    ...
    def _extract_images(self, soup: BeautifulSoup, max_images: int) -> List[Dict[str, str]]:
    ...
    def _generate_scraping_summary(self, result: Dict[str, Any]) -> str:
        ...
        summary_parts = [f"Successfully scraped content from {result.get('title', 'Unknown page')}"]
